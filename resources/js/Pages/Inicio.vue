...
const generarReceta = async (alimentoId = null) => {
    // Verificar si hay alimentos antes de generar
    if (props.total === 0 && !alimentoId) {
        showAlert(
            "Despensa vacía", 
            "No tienes alimentos registrados para crear una receta. ¡Añade algo a tu inventario primero!", 
            "info"
        );
        return;
    }
    ...
    <AuthenticatedLayout>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <Link :href="route('alimentos.index')" class="bg-emerald-vibrant p-6 rounded-[2rem] flex flex-col items-center justify-center text-center group hover:scale-[1.02] transition-all shadow-xl dark:shadow-none min-h-[120px]">
            ...
            <div class="bg-white dark:bg-midnight-card p-8 sm:p-12 rounded-[3rem] shadow-sm dark:shadow-2xl border border-slate-200/60 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden group backdrop-blur-sm">
                ...
                <button 
                    @click="generarReceta()" 
                    :disabled="loading"
                    class="bg-emerald-vibrant hover:bg-emerald-600 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:scale-110 active:scale-95 transition-all disabled:opacity-50 flex items-center gap-3 shrink-0 relative z-10 dark:hover:shadow-2xl"
                >
                ...
                <div class="p-8 bg-slate-50/50 dark:bg-midnight/50 border-t border-slate-50 dark:border-slate-800 flex flex-col items-center gap-6">
                    <div class="flex items-center justify-center gap-4 w-full">
                        <button 
                            ...
                            :class="receta?.es_favorito ? 'bg-rose-500 text-white shadow-lg dark:shadow-rose-500/20 hover:bg-rose-600' : 'bg-white dark:bg-midnight-card text-slate-400 border border-slate-100 dark:border-slate-700 hover:text-rose-500 hover:border-rose-200 dark:hover:border-rose-900/30 hover:bg-rose-50 dark:hover:bg-rose-500/5'"
                        >
                        ...
                        <button 
                            ...
                            class="flex-1 bg-slate-900 dark:bg-emerald-vibrant text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl transition-all active:scale-[0.98] hover:scale-105 hover:bg-slate-800 dark:hover:bg-emerald-600 dark:hover:shadow-emerald-500/20"
                        >
                            Cocinar Ahora
                        </button>
                    </div>
        ...
        <Modal :show="showOnboarding" @close="cerrarOnboarding" maxWidth="sm">
            <div class="p-10 bg-white dark:bg-midnight border border-slate-200/60 dark:border-slate-800/50 rounded-[3rem] text-center">
                <div class="w-20 h-20 bg-emerald-vibrant text-white rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-xl dark:shadow-emerald-500/30">
                ...
                <div class="flex flex-col gap-4">
                    <PrimaryButton @click="irAAyuda" class="w-full justify-center py-5 bg-emerald-vibrant text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl dark:shadow-none">
                        Sí, enséñame
                    </PrimaryButton>
